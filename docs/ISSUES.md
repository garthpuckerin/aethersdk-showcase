...
- **Severity:** medium · **Status:** CLOSED 2026-09-08 — current files resolved 2026-09-05 (scan now covers every tracked file, sealed names matched by hash, docs scrubbed); history rewritten 2026-09-08 on the owner's decision (`git filter-repo` with 13 replacement rules plus removal of the stray presentation media; 44 commits, current tree byte-identical to the verified head, zero history hits for any source string, pack 23 MB → 1.6 MB; `main` force-pushed, house checkout reset). The replacement rules are retained outside this repo (they name the sealed systems). · **Visible:** no
